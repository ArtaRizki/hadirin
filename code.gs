    ...
    for (var d = 6; d >= 0; d--) {
      ...
      for (var j = 1; j < data.length; j++) {
        ...
        if (
          ...
          (s === "Tepat Waktu" || s.startsWith("TL") || s.startsWith("PSW"))
        )
          count++;
      }
...
function getShiftDetails(ss, shiftId, config) {
  if (
    !shiftId ||
    String(shiftId).toUpperCase() === "LIBUR" ||
    String(shiftId).toUpperCase() === "OFF"
  ) {
    return {
      masuk: "00:00",
      pulang: "00:00",
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(shiftId)) {
      return {
        masuk: formatTime(data[i][2], "08:00"),
        pulang: formatTime(data[i][3], "17:00"),
        is_off: false,
        shifting: true,
        shift_name: data[i][1],
      };
  ...
  return {
    masuk: formatTime(config.batasJam, "08:00"),
    pulang: "17:00",
    is_off: false,
    shift_name: "Shift 1",
  };
...
function handleAbsensi(payload) {
  ...
  if (schedule.is_off)
    return responseJSON(
      403,
      "error",
      "Hari ini jadwal Anda LIBUR. Tidak perlu absen.",
    );
  ...
  var interval = parseInt(configData[7]) || 30; // Col H: Interval (default 30)
  var maxTier = parseInt(configData[8]) || 0;  // Col I: Max Tier (0 = unlimited)
...
function handleGetOfficeConfig(payload) {
  ...
  if (!config) return responseJSON(404, "error", "Instansi tidak ditemukan.");
  var ss = SpreadsheetApp.openById(config.spreadsheetId);
  var rawData = ss.getSheetByName("Config_Kantor").getRange("A2:I2").getValues()[0];
...
function formatTime(val, def) {
  ...
  var s = String(val);
  if (s.indexOf(":") !== -1) {
    // Handle ISO String: 2026-04-20T08:00:00.000Z
    if (s.indexOf("T") !== -1) {
      var parts = s.split("T");
      return parts[parts.length - 1].substring(0, 5);
    }
    // Handle Long Date: Sat Dec 30 1899 08:00:00...
...
function savePlottingAssignments(clientId, plottingList) {
  try {
    ...
    for (var i = 1; i < existingData.length; i++) {
      keyRowMap[existingData[i][0]] = i + 1;
    }
