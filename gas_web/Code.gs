...
// KONFIGURASI MASTER
// =============================================================================
const MASTER_API_TOKEN = "LAPASBOJON";
    ...
    for (var d = 6; d >= 0; d--) {
      ...
      for (var j = 1; j < data.length; j++) {
        ...
        if (rDate.getTime() === date.getTime()) {
          ...
          if (s === "Tepat Waktu" || s.startsWith("TL") || s.startsWith("PSW"))
            count++;
        }
      }
...
function getDashboardDataWeb(clientId, id) {
  try {
    ...
    if (now.getHours() < 8) {
      ...
      if (
        !schYest.is_off &&
        toMinutes(schYest.masuk) > toMinutes(schYest.pulang)
      ) {
    ...
    return {
      ...
      schedule: {
        shift_name:
          schedule.shift_name || (schedule.is_off ? "LIBUR" : "Normal"),
        masuk: displayMasuk,
  ...
  try {
    ...
    for (var j = 1; j < logs.length; j++) {
      ...
      if (logTipe === "Masuk") {
        ...
        result[idx].status_absen = logStatus.startsWith("TL")
          ? "Terlambat"
          : "Hadir";
...
function handleGetTodayAttendance(payload) {
  var clientId = String(payload.client_id || "")
    .trim()
    .toUpperCase();
...
function doPost(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "get_shift_list":
        var resShift = getShiftSettings(
          payload.client_id,
          payload.year || 2026,
          payload.month || 4,
        );
        return responseJSON(
          resShift.success ? 200 : 400,
          resShift.success ? "success" : "error",
          resShift,
        );
      case "save_shifts":
        var resSaveS = saveShiftDefinitions(
          payload.client_id,
          payload.shift_list,
        );
        return responseJSON(
          resSaveS.success ? 200 : 400,
          resSaveS.success ? "success" : "error",
          resSaveS.message,
        );
      case "save_plotting":
        var resSaveP = savePlottingAssignments(
          payload.client_id,
          payload.plotting_list,
        );
        return responseJSON(
          resSaveP.success ? 200 : 400,
          resSaveP.success ? "success" : "error",
          resSaveP.message,
        );
      case "update_default_shift":
        var resUpdD = updateDefaultShift(
          payload.client_id,
          payload.id_karyawan_target,
          payload.new_shift_id,
        );
        return responseJSON(
          resUpdD.success ? 200 : 400,
          resUpdD.success ? "success" : "error",
          resUpdD.message,
        );
      ...
      default:
        return responseJSON(400, "error", "Action Unknown: " + action);
    }
  } catch (err) {
...
function getEffectiveSchedule(ss, config, idKaryawan, targetDate) {
  ...
  if (sheetJadwal) {
    ...
    for (var i = 1; i < dataJadwal.length; i++) {
      ...
      if (!tglCell) continue;
      var tglStr =
        tglCell instanceof Date
          ? Utilities.formatDate(tglCell, "GMT+7", "yyyy-MM-dd")
          : String(tglCell);
      if (tglStr === todayStr) {
        return {
          masuk: formatTime(dataJadwal[i][1], "08:00"),
          pulang: formatTime(dataJadwal[i][2], "17:00"),
          is_khusus: true,
          keterangan: dataJadwal[i][3],
          is_off: String(dataJadwal[i][1]).toUpperCase() === "LIBUR",
        };
      }
    }
  }
  ...
  var configData = ss
    ...
    .getValues()[0];
  return {
    masuk: formatTime(configData[5], config.batasJam),
    pulang: formatTime(configData[6], "17:00"),
    is_khusus: false,
    is_off: false,
  };
}
...
function getShiftDetails(ss, shiftId, config) {
  if (
    !shiftId ||
    ["LIBUR", "OFF"].indexOf(String(shiftId).toUpperCase()) !== -1
  ) {
    return {
      masuk: "00:00",
      pulang: "00:00",
...
function handleAbsensi(payload) {
  ...
  if (now.getHours() < 10) {
    var yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    var schYest = getEffectiveSchedule(
      ss,
      config,
      payload.id_karyawan,
      yesterday,
    ...
    var isYestOvernight =
      !schYest.is_off && toMinutes(schYest.masuk) > toMinutes(schYest.pulang);
  ...
  if (payload.foto_base64) {
    try {
      ...
      var blob = Utilities.newBlob(
        ...
        "image/jpeg",
        "Absen_" + payload.id_karyawan + "_" + Date.now() + ".jpg",
      );
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === targetId) {
      ...
      if (!faceData || faceData === "")
        return responseJSON(404, "error", "Face not registered");
  ...
  if (payload.foto_base64) {
    try {
      ...
      var blob = Utilities.newBlob(
        Utilities.base64Decode(payload.foto_base64),
        "image/jpeg",
        "Izin_" + payload.id_karyawan + "_" + Date.now() + ".jpg",
      );
      var file = folder.createFile(blob);
      file.setSharing(
        DriveApp.Access.ANYONE_WITH_LINK,
...
function handleGetHistory(payload) {
  ...
  for (var i = 1; i < logs.length; i++)
    if (String(logs[i][1]) === String(payload.id_karyawan))
      res.push({ waktu: logs[i][0], tipe: logs[i][2], status: logs[i][6] });
...
function handleGetOfficeConfig(payload) {
  ...
  var resp = {
    ...
    lng: raw[2],
    radius: raw[3],
    jam_masuk_mulai: formatTime(raw[4], "04:00"),
    batas_jam_masuk: formatTime(raw[5], "07:00"),
    jam_pulang_mulai: formatTime(raw[6], "13:00"),
    tl_interval: parseInt(raw[7]) || 30,
    max_tier: parseInt(raw[8]) || 0,
  };
  if (payload.id_karyawan) {
    ...
    for (var i = 1; i < masterData.length; i++) {
      if (String(masterData[i][0]).trim().toLowerCase() === tid) {
        resp.wajah_terdaftar = !!(
          masterData[i][4] && String(masterData[i][4]).length > 20
        );
        break;
      }
    }
...
function handleGetLeaveHistory(payload) {
  ...
  logs.forEach((r) => {
    if (
      String(r[1]) === String(payload.id_karyawan) &&
      ["Izin", "Sakit", "Cuti"].indexOf(String(r[2])) !== -1
    )
      res.push({ waktu: r[0], tipe: r[2], status: r[6] });
  });
  return responseJSON(200, "success", res.reverse());
}
...
function handleGetMonthlyReport(payload) {
  ...
  var namaMap = {};
  for (var j = 1; j < employees.length; j++) {
    namaMap[String(employees[j][0]).trim().toLowerCase()] = String(
      employees[j][1],
    );
  }
  ...
  logs.slice(1).forEach((r) => {
    ...
    if (
      b === payload.bulan_tahun &&
      (payload.id_karyawan_target === "SEMUA" ||
        String(r[1]) === payload.id_karyawan_target)
    ) {
...
function getShiftSettings(clientId, year, month) {
  ...
  for (var i = 1; i < plots.length; i++) {
    var tgl =
      plots[i][1] instanceof Date
        ? Utilities.formatDate(plots[i][1], "GMT+7", "yyyy-MM-dd")
        : String(plots[i][1]);
...
function savePlottingAssignments(clientId, list) {
  var sheet = SpreadsheetApp.openById(
    getSemuaConfig()[clientId].spreadsheetId,
  ).getSheetByName("Jadwal_Shift");
  var existing = sheet.getDataRange().getValues();
  var map = {};
  existing.forEach((r, idx) => (map[r[0]] = idx + 1));
  list.forEach((p) => {
...
function updateDefaultShift(clientId, id, shiftId) {
  ...
  for (var i = 1; i < data.length; i++)
    if (String(data[i][0]) === String(id)) {
      sheet.getRange(i + 1, 7).setValue(shiftId);
      return { success: true };
    }
