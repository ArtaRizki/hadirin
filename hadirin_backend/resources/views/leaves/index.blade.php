...
<div class="content-view fade-in">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px;">
        ...
        <div class="card glass" style="padding: 24px; display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px; gap: 10px;">
                <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
                    <div style="width: 40px; height: 40px; min-width: 40px; border-radius: 50%; background: var(--primary-gradient); color: white; display: flex; align-items: center; justify-content: center;">
                        <i data-lucide="user"></i>
                    </div>
                    <div style="min-width: 0;">
                        <div style="font-weight: 800; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $leave->user->name ?? '-' }}</div>
                        <div style="font-size: 0.75rem; color: var(--text-muted); font-weight: 600;">{{ $leave->created_at->format('d M Y H:i') }}</div>
                    </div>
                </div>
                <span class="badge-tipe" style="background: rgba(245, 158, 11, 0.1); color: #f59e0b; flex-shrink: 0;">{{ $leave->type }}</span>
            </div>

            <div style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 8px;">
                <strong>Waktu:</strong> {{ $leave->lat_long ?? '-' }}
            </div>
            <div style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 15px; padding: 12px; background: rgba(0,0,0,0.02); border-radius: 8px; line-height: 1.5;">
                {{ $leave->reason }}
            </div>
            ...
            @else
                <div style="text-align: center; padding: 10px; font-weight: 700; border-radius: 8px; margin-top: auto;
                    background: {{ $leave->leave_status == 'Disetujui' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)' }};
                    color: {{ $leave->leave_status == 'Disetujui' ? '#10b981' : '#ef4444' }};">
                    {{ $leave->leave_status }}
                </div>
            @endif
        </div>
